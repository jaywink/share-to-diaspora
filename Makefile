...
# 1) run make once to create and update the po/ubuntu-diaspora.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/ubuntu-diaspora.pot po/en.po
# 3) edit the po file
...
APP_ID=share-to-diaspora.jaywink
...
po/share-to-diaspora.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/share-to-diaspora.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
